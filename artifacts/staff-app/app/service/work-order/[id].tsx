...
export default function WorkOrderDetailScreen() {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    if (newStatus === "completed") {
      ...
      const completeConfirmMsg =
        queuedRetries > 0
          ? t("serviceModule.completeConfirmWithRetries", {
            ...
            if (order.assetId && isQueueable("change_maintenance_status")) {
              await enqueue({
                ...
                payload: {
                  ...
                  logType:
                    order.orderType === "scheduled_maintenance"
                      ? "general_service"
                      : "other",
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={[styles.section, { backgroundColor: colors.card }]}>
          ...
          {order.actualCost && (
            <Row
              label={t("serviceModule.actualCost")}
              value={`${parseFloat(order.actualCost).toLocaleString("ru-RU")} ₽`}
              colors={colors}
            />
          )}
        </View>
