  ...
  const renderItem = ({ item }: { item: SparePart }) => (
    <View style={[styles.card, { backgroundColor: colors.card }]}>
      <View style={styles.cardTop}>
        <View style={{ flex: 1 }}>
          <Text
            style={[styles.partName, { color: colors.foreground }]}
            numberOfLines={1}
          >
            {item.name}
          </Text>
          {item.sku && (
            <Text style={[styles.partSku, { color: colors.mutedForeground }]}>
              SKU: {item.sku}
            </Text>
          )}
        </View>
        <View style={styles.stockBlock}>
          <Text
            style={[
              styles.stockQty,
              { color: isLow(item) ? "#ef4444" : "#22c55e" },
            ]}
          >
            {parseFloat(item.qtyInStock).toLocaleString("ru-RU")}
          </Text>
          <Text style={[styles.stockUnit, { color: colors.mutedForeground }]}>
            {item.unit}
          </Text>
          {isLow(item) && (
      ...
      <View style={styles.cardMeta}>
        <Text style={[styles.metaText, { color: colors.mutedForeground }]}>
          {t(`serviceModule.cat_${item.category}`, {
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
            setTxModal({ part: item, type: "adjustment" });
            setQty(item.qtyInStock);
            setTxNote("");
          }}
        >
          <Feather name="edit-2" size={14} color={colors.primary} />
          <Text style={[styles.actionText, { color: colors.primary }]}>
            {t("serviceModule.adjust")}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <TouchableOpacity
          ...
          style={[
            styles.filterBtn,
            lowStockOnly && { backgroundColor: YELLOW + "30" },
          ]}
        >
          <Feather
            name="alert-triangle"
            size={18}
            color={lowStockOnly ? YELLOW : "#ffffff80"}
          />
        </TouchableOpacity>
      </View>
        ...
        >
          <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
            <Text style={[styles.modalTitle, { color: colors.foreground }]}>
              {txModal?.type === "in"
                ? t("serviceModule.receiveTitle")
                : txModal?.type === "out"
                  ? t("serviceModule.issueTitle")
                  : t("serviceModule.adjustTitle")}
            </Text>
            <Text style={[styles.modalPart, { color: colors.mutedForeground }]}>
              {txModal?.part?.name}
            </Text>
