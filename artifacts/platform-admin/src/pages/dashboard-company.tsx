    ...
    >
      ...
      <CardContent className="pt-5 pl-5">
        <div className="flex items-start justify-between">
          ...
          <div
            className={`p-2.5 rounded-xl ${accent.replace("bg-", "bg-").replace("-500", "-100")} mt-0.5`}
          >
    ...
    <div className="p-6 space-y-6 max-w-7xl">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">
            {companyName || t("companyDashboard.title")}
          </h1>
          ...
          <CardContent className="pt-6">
            <Empty className="border-0 p-0">
              <EmptyHeader>
                ...
                <EmptyDescription>
                  {t("companyDashboard.welcomeDescription")}
                </EmptyDescription>
              </EmptyHeader>
        ...
        <KpiCard
          ...
          sub={
            overdueRentals.length > 0
              ? `${overdueRentals.length} ${t("status.overdue")}`
              : undefined
          }
          isLoading={isLoading}
      ...
      {overdueRentals.length > 0 && (
        <div className="flex items-center gap-3 bg-orange-50 border border-orange-200 rounded-xl px-4 py-3">
          ...
          <p className="text-sm text-orange-800 flex-1">
            <span className="font-semibold">{overdueRentals.length}</span>{" "}
            {t(
              "companyDashboard.overdueAlert",
              "аренд просрочено — требуют внимания",
            )}
          </p>
          <Button
            size="sm"
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-semibold">
              {t("companyDashboard.assetsByStatus")}
            </CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
                    ...
                    <PieChart>
                      ...
                      <Tooltip
                        formatter={(val: number, name: string) => [
                          `${val} (${Math.round((val / assets.length) * 100)}%)`,
                          name,
                        ]}
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-semibold">
              {t("companyDashboard.fleetHealth", "Состояние парка")}
            </CardTitle>
          </CardHeader>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">
                      {t("companyDashboard.utilization", "Загрузка парка")}
                    </span>
      ...
      <Card>
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
          <CardTitle className="text-base font-semibold">
            {t("companyDashboard.recentRentals")}
          </CardTitle>
          <Button
            variant="ghost"
            size="sm"
              ...
              <TableBody>
                {recentRentals.map((rental) => (
                  <TableRow key={rental.id} className="hover:bg-muted/30">
                    ...
                    <TableCell>
                      <Badge
                        className={`text-xs ${STATUS_COLORS[rental.status] || "bg-gray-100"}`}
                      >
