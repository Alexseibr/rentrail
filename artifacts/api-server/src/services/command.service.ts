...
export async function enqueueCommand(companyId: string, deviceId: string, data: {
  ...
  const [device] = await db.select().from(devices)
    .where(and(eq(devices.id, deviceId), eq(devices.companyId, companyId))).limit(1);
  ...
  if (device.capabilities) {
    ...
    if (Array.isArray(caps) && caps.length > 0 && !caps.includes(data.commandType)) {
      throw new AppError(422, `Device does not support command '${data.commandType}'`, "UNSUPPORTED_COMMAND");
    }
  }

  const expiresAt = data.expiresInMinutes ? new Date(Date.now() + data.expiresInMinutes * 60000) : null;
...
export async function getCommand(id: string, companyId: string) {
  const [cmd] = await db.select().from(deviceCommands)
    .where(and(eq(deviceCommands.id, id), eq(deviceCommands.companyId, companyId))).limit(1);
...
export async function listDeviceCommands(deviceId: string, companyId: string) {
  return db.select().from(deviceCommands)
    .where(and(eq(deviceCommands.deviceId, deviceId), eq(deviceCommands.companyId, companyId)))
...
export async function updateCommandStatus(id: string, companyId: string, newStatus: string, extra?: { responsePayload?: unknown; errorMessage?: string }) {
  const cmd = await getCommand(id, companyId);
  const allowed = COMMAND_STATUS_TRANSITIONS[cmd.status];
  if (!allowed || !allowed.includes(newStatus)) {
    throw new AppError(422, `Cannot transition command from '${cmd.status}' to '${newStatus}'`, "INVALID_STATUS_TRANSITION");
  }
...
export async function enqueueAssetCommand(companyId: string, assetId: string, commandType: string, userId?: string) {
  ...
  const bindings = await db.select({ deviceId: assetDevices.deviceId, bindingType: assetDevices.bindingType, isPrimary: assetDevices.isPrimary })
    .from(assetDevices)
    .where(and(eq(assetDevices.assetId, assetId), eq(assetDevices.companyId, companyId), eq(assetDevices.status, "active")));

  let targetDeviceId: string | null = null;
  const primaryMatch = bindings.find(b => b.bindingType === neededType && b.isPrimary);
  if (primaryMatch) {
  ...
  if (!targetDeviceId) {
    throw new AppError(422, `No suitable ${neededType} device bound to this asset`, "NO_DEVICE_BOUND");
  }

  return enqueueCommand(companyId, targetDeviceId, {
    commandType, assetId, requestedByUserId: userId, expiresInMinutes: 10,
  });
}
