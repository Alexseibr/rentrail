...
export async function bindDeviceToAsset(
  companyId: string,
  assetId: string,
  data: {
    deviceId: string;
  ...
  const [device] = await db
    ...
    .from(devices)
    .where(and(eq(devices.id, data.deviceId), eq(devices.companyId, companyId)))
    .limit(1);
  if (!device)
    throw new NotFoundError(
      "Device not found or belongs to a different company",
    );
  ...
  const existingActive = await db
    ...
    .where(
      and(
        ...
        eq(assetDevices.deviceId, data.deviceId),
  ...
  if (existingActive.length > 0)
    throw new AppError(
      409,
      "Device is already bound to this asset",
      "ALREADY_BOUND",
    );
  ...
  if (data.isPrimary) {
    await db
      ...
      .where(
        and(
          eq(assetDevices.assetId, assetId),
          eq(assetDevices.bindingType, data.bindingType as BindingType),
          eq(assetDevices.status, "active"),
        ),
      );
  }
...
export async function removeBinding(
  ...
  if (binding.status === "removed")
    throw new AppError(409, "Binding already removed", "ALREADY_REMOVED");

  const [updated] = await db
    .update(assetDevices)
    .set({ status: "removed", removedAt: new Date(), updatedAt: new Date() })
    .where(eq(assetDevices.id, bindingId))
    .returning();
  return updated;
}
...
export async function setPrimaryBinding(
  ...
  await db
    ...
    .where(
      and(
        eq(assetDevices.assetId, assetId),
        eq(assetDevices.bindingType, binding.bindingType),
        eq(assetDevices.status, "active"),
      ),
    );
...
export async function getCurrentActiveDevicesForAsset(
  assetId: string,
  companyId: string,
) {
