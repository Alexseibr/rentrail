...
export async function listDevices(
  ...
  if (filters?.status)
    conditions.push(eq(devices.status, filters.status as DeviceStatus));
  if (filters?.deviceType)
    conditions.push(eq(devices.deviceType, filters.deviceType as DeviceType));
  if (filters?.branchId)
    conditions.push(eq(devices.branchId, filters.branchId));
  if (filters?.provider)
    conditions.push(eq(devices.provider, filters.provider));
  return db
    .select()
    .from(devices)
...
export async function changeDeviceStatus(
  ...
  if (!allowed || !allowed.includes(newStatus)) {
    throw new AppError(
      422,
      `Cannot transition device from '${device.status}' to '${newStatus}'`,
      "INVALID_STATUS_TRANSITION",
    );
  }
  const [updated] = await db
    .update(devices)
    .set({ status: newStatus as DeviceStatus, updatedAt: new Date() })
    .where(and(eq(devices.id, id), eq(devices.companyId, companyId)))
    .returning();
  return updated;
}
...
export async function archiveDevice(id: string, companyId: string) {
  ...
  if (device.archivedAt)
    throw new AppError(409, "Device already archived", "ALREADY_ARCHIVED");
  const [updated] = await db
    .update(devices)
...
export async function restoreDevice(id: string, companyId: string) {
  const device = await getDevice(id, companyId);
  if (!device.archivedAt)
    throw new AppError(409, "Device is not archived", "NOT_ARCHIVED");
  const [updated] = await db
    .update(devices)
...
export async function getActiveBindingsForDevice(
  deviceId: string,
  companyId: string,
) {
  return db
    ...
    .where(
      and(
        eq(assetDevices.deviceId, deviceId),
