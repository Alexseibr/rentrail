import { db, assets, telemetrySnapshots, devices, batteries, batteryAssignments } from "@workspace/db";
import { eq, and, desc, lt, isNull, lte } from "drizzle-orm";

export async function getLiveMapData(companyId: string, filters?: { branchId?: string; assetType?: string; status?: string }) {
  ...
  for (const asset of filtered) {
    const [snap] = await db.select({
      ...
      .where(and(eq(telemetrySnapshots.assetId, asset.id), eq(telemetrySnapshots.companyId, companyId)))
...
export async function getLowBatteryAssets(companyId: string, threshold = 20) {
  const snaps = await db.select({
    ...
    .where(and(eq(telemetrySnapshots.companyId, companyId), lte(telemetrySnapshots.batteryPercent, threshold)))
    .orderBy(desc(telemetrySnapshots.recordedAt)).limit(100);
...
export async function getOfflineDevices(companyId: string, thresholdMinutes = 30) {
  ...
  return db.select({
    ...
    .where(and(eq(devices.companyId, companyId), eq(devices.status, "active"), lt(devices.lastSeenAt, cutoff)));
}
