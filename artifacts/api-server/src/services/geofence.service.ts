...
export async function getGeofence(id: string, companyId: string) {
  const [geo] = await db
    .select()
    .from(geofences)
    .where(and(eq(geofences.id, id), eq(geofences.companyId, companyId)))
    .limit(1);
...
export async function listGeofences(
  companyId: string,
  filters?: { type?: string; branchId?: string; isActive?: boolean },
) {
  const conditions = [
    eq(geofences.companyId, companyId),
    isNull(geofences.archivedAt),
  ];
  if (filters?.type)
    conditions.push(eq(geofences.type, filters.type as GeofenceType));
  if (filters?.branchId)
    conditions.push(eq(geofences.branchId, filters.branchId));
...
export async function archiveGeofence(id: string, companyId: string) {
  ...
  if (geo.archivedAt)
    throw new AppError(409, "Geofence already archived", "ALREADY_ARCHIVED");
  const [updated] = await db
    .update(geofences)
...
export function evaluatePointAgainstGeofences(
  lat: number,
  lng: number,
  geoList: (typeof geofences.$inferSelect)[],
) {
  const results: Array<{
    geofenceId: string;
    name: string;
