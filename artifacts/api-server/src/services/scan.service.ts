...
export async function resolveScannedCode(code: string, companyId: string): Promise<ScanResult> {
  const normalized = code.trim();

  const [assetByQr] = await db.select().from(assets).where(
    and(eq(assets.companyId, companyId), eq(assets.qrCode, normalized)),
  );
  if (assetByQr) return { type: "asset", entity: assetByQr };

  const [assetByInternal] = await db.select().from(assets).where(
  ...
  const [assetBySerial] = await db.select().from(assets).where(
