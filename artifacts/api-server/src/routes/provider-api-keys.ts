...
router.post("/provider-api-keys", authenticate, requireCompanyAccess, requirePermission("settings:manage"),
  validate({ body: createKeySchema }), async (req, res) => {
    const result = await keyService.generateApiKey(req.tenant!.companyId, req.body);
    await createAuditLog({ companyId: req.tenant!.companyId, actorUserId: req.user!.userId, action: "create", entityType: "provider_api_key", entityId: result.id, req });
    res.status(201).json({ data: result });
  });

router.get("/provider-api-keys", authenticate, requireCompanyAccess, requirePermission("settings:read"), async (req, res) => {
