...
const VALID_INCIDENT_STATUSES = ["open", "in_progress", "resolved", "closed"] as const;

router.get("/incidents", authenticate, requireCompanyAccess, requirePermission("asset:read"), async (req, res) => {
  try {
    const statusParam = req.query.status as string | undefined;
    if (statusParam && !VALID_INCIDENT_STATUSES.includes(statusParam as (typeof VALID_INCIDENT_STATUSES)[number])) {
...
router.post("/incidents", authenticate, requireCompanyAccess, requirePermission("asset:update"), async (req, res) => {
  try {
    if (!req.body.title) {
...
router.post("/incidents/:id/status", authenticate, requireCompanyAccess, requirePermission("asset:update"), async (req, res) => {
  try {
