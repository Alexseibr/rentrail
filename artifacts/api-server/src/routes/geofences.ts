...
const createGeofenceSchema = z.object({
  ...
  type: z.enum(["operating_zone", "no_ride_zone", "return_zone", "service_zone", "charging_zone"]),
...
const updateGeofenceSchema = z.object({
  ...
  type: z.enum(["operating_zone", "no_ride_zone", "return_zone", "service_zone", "charging_zone"]).optional(),
  geometry: z.object({ type: z.string(), coordinates: z.array(z.unknown()) }).optional(),
  rules: z.record(z.string(), z.unknown()).nullable().optional(),
  isActive: z.boolean().optional(),
});

router.post("/geofences", authenticate, requireCompanyAccess, requirePermission("geofence:create"),
  validate({ body: createGeofenceSchema }), async (req, res) => {
    const geo = await geofenceService.createGeofence(req.tenant!.companyId, req.body);
    await createAuditLog({ companyId: req.tenant!.companyId, actorUserId: req.user!.userId, action: "create", entityType: "geofence", entityId: geo.id, req });
    res.status(201).json({ data: geo });
  });

router.get("/geofences", authenticate, requireCompanyAccess, requirePermission("geofence:read"), async (req, res) => {
  ...
  const list = await geofenceService.listGeofences(req.tenant!.companyId, {
    type, branchId, isActive: isActive !== undefined ? isActive === "true" : undefined,
  });
  res.json({ data: list });
});
