...
router.get(
  ...
  async (req, res) => {
    const { status, deviceType, branchId, provider } = req.query as Record<
      string,
      string
    >;
    if (
      status &&
      !VALID_DEVICE_STATUSES.includes(
        status as (typeof VALID_DEVICE_STATUSES)[number],
      )
    ) {
    ...
    if (
      ...
      !VALID_DEVICE_TYPES.includes(
        deviceType as (typeof VALID_DEVICE_TYPES)[number],
      )
    ) {
      return res.status(400).json({
        error: {
          code: "VALIDATION",
          message: `Invalid deviceType value: ${deviceType}`,
        },
      });
    }
    const list = await deviceService.listDevices(req.tenant!.companyId, {
      status,
      deviceType,
      branchId,
      provider,
...
router.patch(
  ...
  validate({ params: idParams, body: updateDeviceSchema }),
  async (req, res) => {
...
router.post(
  ...
  validate({ params: idParams }),
  async (req, res) => {
    const device = await deviceService.archiveDevice(
      req.params.id as string,
      req.tenant!.companyId,
    );
    await createAuditLog({
...
router.post(
  ...
  validate({ params: idParams }),
  async (req, res) => {
    const device = await deviceService.restoreDevice(
      req.params.id as string,
      req.tenant!.companyId,
    );
    await createAuditLog({
