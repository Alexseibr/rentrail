...
describe("GET /api/work-orders — status filter", () => {
  ...
  beforeAll(async () => {
    ...
    woDraft = rb(resDraft).data.id as string;
    ...
    woInProgress = rb(resInProgress).data.id as string;
    ...
    woCompleted = rb(resCompleted).data.id as string;
  ...
  it("status=draft — returns only draft work orders", async () => {
    ...
    const ids = (rb(res).data as unknown as Array<Record<string, unknown>>).map(
      (wo: Record<string, unknown>) => wo.id as string,
    );
  ...
  it("each filtered result has the correct status field", async () => {
    ...
    expect(res.status).toBe(200);
    for (const wo of rb(res).data as unknown as Array<
...
describe("GET /api/work-orders — branchId filter", () => {
  ...
  beforeAll(async () => {
    ...
    woBranchA = rb(resA).data.id as string;
    ...
    woBranchB = rb(resB).data.id as string;
    ...
    woNoBranch = rb(resNone).data.id as string;
  }, 30000);
...
describe("GET /api/work-orders — combined status + branchId filter", () => {
  ...
  beforeAll(async () => {
    ...
    woBranchADraft = rb(resADraft).data.id as string;
    ...
    woBranchACompleted = rb(resACompleted).data.id as string;
    ...
    woBranchBDraft = rb(resBDraft).data.id as string;
  }, 30000);
