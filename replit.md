...
**Type Coverage:** `pnpm run type-coverage` measures the percentage of TypeScript nodes that are explicitly typed (not `any`) across all three artifact packages. Baselines measured 2026-05-09: `api-server` 99.72% (threshold ≥ 99.72), `platform-admin` 100.00% (threshold ≥ 100), `staff-app` 100.00% (threshold ≥ 100). The check runs as part of `pnpm run ci` after `typecheck`, failing the build on regression. Thresholds are intentionally set 0-1% below baseline to tolerate rounding; raise them as `any` usages are eliminated.
