...
**Type Coverage:** `pnpm run type-coverage` measures the percentage of TypeScript nodes that are explicitly typed (not `any`) across all three artifact packages. Baselines measured 2026-05-08: `api-server` 95.59% (threshold ≥ 99, raised 2026-05-09), `platform-admin` 98.98% (threshold ≥ 98), `staff-app` 99.51% (threshold ≥ 99, raised 2026-05-09). The check runs as part of `pnpm run ci` after `typecheck`, failing the build on regression. Thresholds are intentionally set 0-1% below baseline to tolerate rounding; raise them as `any` usages are eliminated.
