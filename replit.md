...
**Type Coverage:** `pnpm run type-coverage` measures the percentage of TypeScript nodes that are explicitly typed (not `any`) across all three artifact packages. All three packages score 100.00% (threshold ≥ 100). The check runs as part of `pnpm run ci` after `typecheck`, failing the build on regression. Route handlers that access `req.body` (typed as `any` by Express) and test assertions that read `res.body` (typed as `any` by supertest) are marked with `// type-coverage:ignore-next-line` since the developer-facing type is explicit via `as` casts — adding any new untyped node will now cause a coverage regression.
